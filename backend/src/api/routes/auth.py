...
async def login_for_access_token(
    ...
    if not response.data or not verify_password(
        form_data.password, response.data.get("hashed_password", "")
    ):
        raise HTTPException(
    ...
    access_token = create_access_token(
        data={"sub": str(response.data["id"])}, expires_delta=access_token_expires
    )
...
async def register_user(
    email: str, password: str, username: str, db: AsyncSession = Depends(get_db)
) -> dict[str, Any]:
    """
    Register a new user
    """
