...
class RenderEngineManager:
    ...
    def validate_engine_settings(self, engine_type: RenderEngineType, settings: Dict[str, Any]) -> bool:
    ...
    def update_job_progress(self, job_id: str, progress: int, status: RenderStatus = None) -> bool:
    ...
    def complete_job(self, job_id: str, result: RenderResult) -> bool:
        ...
        if job:
            job.result = result
            job.status = RenderStatus.COMPLETED if result.success else RenderStatus.FAILED
