        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "--render-anim",
                    "--output",
                    os.path.join(self.temp_dir, "render_"),
                    "--render-frame",
                    "1",  # Render first frame to create scene file
                ],
                capture_output=True,
                text=True,
                timeout=60,
    ...
    def render_video(
        ...
        try:
            if progress_callback:
                progress_callback(
                    0, RenderStatus.INITIALIZING, "Starting Blender render"
                )
            ...
            if result.returncode == 0:
                if progress_callback:
                    progress_callback(
                        90, RenderStatus.POST_PROCESSING, "Post-processing"
                    )
