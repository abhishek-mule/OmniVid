...
class FfmpegRenderEngine(RenderEngine):
    ...
    def _generate_ffmpeg_script(self, prompt: str, settings: Dict[str, Any], config_path: str) -> str:
    ...
    def _build_ffmpeg_command(self, settings: Dict[str, Any], output_path: str) -> List[str]:
        ...
        cmd.extend([
            "-f", "lavfi",
            "-i", f"color=c={color}:s={resolution[0]}x{resolution[1]}:d={duration}"
        ])
        ...
        if "text" in str(settings).lower():
            ...
            filters.append(f"drawtext=text='{text}':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2")
