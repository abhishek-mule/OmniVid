...
class RemotionRenderEngine(RenderEngine):
    ...
    def create_scene(self, prompt: str, settings: Dict[str, Any]) -> str:
        ...
        try:
            ...
            subprocess.run(init_cmd, cwd=self.temp_dir, capture_output=True, timeout=60)
    ...
    def render_video(
        ...
        try:
            if progress_callback:
                progress_callback(
                    0, RenderStatus.INITIALIZING, "Starting Remotion render"
                )
            ...
            if progress_callback:
                progress_callback(
                    40, RenderStatus.RENDERING, "Compiling React components"
                )

            # Build Remotion project
            build_cmd = self.remotion_path.split() + [
                "render",
                "Video",
                output_path,
                "--concurrency",
    ...
    def _generate_react_components(
...
"""
        ...
        return {"main": main_component, "other": other_components}
    ...
    def _ensure_dependencies(self):
        ...
        if os.path.exists(package_json):
            ...
            subprocess.run(
                ["npm", "install"], cwd=self.temp_dir, capture_output=True, timeout=120
            )
