...
class TestVideoGenerationWorkflow:
    ...
    def test_complete_video_workflow(self, client, db, test_project_and_video):
        ...
        with patch('src.workers.tasks.video_processing.generate_video.delay') as mock_task:
            ...
            video_repo = VideoRepository(db)
            video_repo.update_video(video.id, {"status": "processing", "celery_task_id": "test-celery-task-id"})
        ...
        video_repo.update_video(video.id, {
    ...
    def test_websocket_progress_broadcast(self, client, db, test_project_and_video):
        ...
        response = client.post(
            "/ws/test/broadcast",
            json={"video_id": str(video.id), "progress": 75, "stage": "Rendering frames"}
        )
        assert response.status_code == 200
    ...
    def test_video_crud_operations(self, client, db, test_project_and_video):
        ...
        response = client.put(f"/api/videos/{video.id}", json=update_data, headers=headers)
        assert response.status_code == 200
    ...
    def test_multi_video_workflow(self, client, db, test_user_and_token):
        ...
        project = project_repo.create_project(
            user.id,
            ProjectCreate(title="Multi-Video Test Project", is_public=True)
        )
    ...
    def test_project_isolation(self, client, db):
        ...
        user1 = user_repo.create_user(UserCreate(
            email="user1@example.com", username="user1", password="password"
        ))
        user2 = user_repo.create_user(UserCreate(
        ...
        private_project = project_repo.create_project(
            user1.id, 
            ProjectCreate(title="Private Project", is_public=False)
        )
        
        # User2 creates a public project
        public_project = project_repo.create_project(
            user2.id,
            ProjectCreate(title="Public Project", is_public=True)
        )
        
        # User1 can see their private project
        response = client.get(f"/api/projects/{private_project.id}", 
        ...
        response = client.get(f"/api/projects/{public_project.id}",
                            headers={"Authorization": f"Bearer {token2}"})
        assert response.status_code == 200
...
class TestDatabaseIntegrity:
    ...
    def test_video_deletion_constraints(self, client, db, test_project_and_video):
        ...
        assert "Cannot delete video that is currently processing" in response.json()["detail"]
...
class TestPerformance:
    ...
    def test_multiple_concurrent_videos(self, client, db, test_user_and_token):
        ...
        project = project_repo.create_project(
            user.id,
            ProjectCreate(title="Load Test Project", is_public=True)
        )
