...
def test_get_projects(db, test_user_and_token):
    ...
    project1 = project_repo.create_project(user.id, ProjectCreate(title="Project 1", is_public=True))
    project2 = project_repo.create_project(user.id, ProjectCreate(title="Project 2", is_public=False))
...
def test_get_project_by_id(db, test_user_and_token):
    ...
    # Create a project
    project_repo = ProjectRepository(db)
    project = project_repo.create_project(user.id, ProjectCreate(title="Test Project", is_public=True))
...
def test_update_project(db, test_user_and_token):
    ...
    # Create a project
    project_repo = ProjectRepository(db)
    project = project_repo.create_project(user.id, ProjectCreate(title="Original Title", is_public=True))
    ...
    response = client.put(f"/api/projects/{project.id}", json=update_data, headers=headers)
...
def test_get_videos(db, test_user_and_token):
    ...
    video1 = video_repo.create_video(VideoCreate(
        title="Video 1", prompt="First video", project_id=project.id
    ))
    video2 = video_repo.create_video(VideoCreate(
        title="Video 2", prompt="Second video", project_id=project.id
    ))
...
def test_get_video_by_id(db, test_user_and_token):
    ...
    video_repo = VideoRepository(db)
    video = video_repo.create_video(VideoCreate(
        title="Test Video", prompt="Test prompt", project_id=project.id
    ))
...
def test_create_video_without_permission(db):
    ...
    # Create two users
    user_repo = UserRepository(db)
    user1 = user_repo.create_user(UserCreate(email="user1@example.com", username="user1", password="password"))
    user2 = user_repo.create_user(UserCreate(email="user2@example.com", username="user2", password="password"))
    ...
    project_repo = ProjectRepository(db)
    project = project_repo.create_project(user1.id, ProjectCreate(title="User1 Project", is_public=False))
...
def test_access_public_project(db, test_user_and_token):
    ...
    user_repo = UserRepository(db)
    other_user = user_repo.create_user(UserCreate(
        email="other@example.com", username="otheruser", password="password"
    ))
    
    project_repo = ProjectRepository(db)
    public_project = project_repo.create_project(other_user.id, ProjectCreate(
        title="Public Project", is_public=True
    ))
