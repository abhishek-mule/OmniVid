...
from ..src.render_engines.base import RenderEngineManager, RenderEngineType, RenderResult, RenderStatus
...
def test_get_render_status(render_pipeline, temp_output_dir):
    ...
    with patch.object(render_pipeline.render_manager, 'get_job_status', return_value=mock_job_data):
...
def test_get_all_render_jobs(render_pipeline):
    ...
    with patch.object(render_pipeline.render_manager, 'get_all_jobs', return_value=mock_jobs):
...
def test_cleanup_old_jobs(render_pipeline):
    ...
    with patch.object(render_pipeline.render_manager, 'cleanup_completed_jobs', return_value=5):
...
def test_start_render_with_invalid_engine(render_pipeline):
    ...
    with patch.object(render_pipeline.render_manager, 'get_available_engines', return_value=[]):
        with pytest.raises(ValueError, match="Engine .* is not available"):
            render_pipeline.start_render(
                prompt, settings, output_path, 
                engine_type=RenderEngineType.BLENDER
            )
